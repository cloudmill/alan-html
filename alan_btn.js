(function (ns) {
  ...
  function alanBtnFake(options) {
    ...
    if (window.tutorProject && !isTutorMode()) {
      throw new Error(
        "The Alan Button instance has already been created. There cannot be two Alan Button instances created at the same time"
      );
    }
    ...
    var NO_VOICE_SUPPORT_IN_BROWSER_MSG =
    ...
    var NO_VOICE_SUPPORT_IN_BROWSER_CODE =
    ...
    var NO_ALAN_AUDIO_INSANCE_WAS_PROVIDED_CODE =
    ...
    function setButtonPosition(keepBtnPosition) {
      ...
      if (isLeftAligned) {
        sideBtnPos = setDefautlPositionProps(
          options.left !== undefined ? options.left : btnModes[mode].leftPos
        );
      } else {
        sideBtnPos = setDefautlPositionProps(
          options.right !== undefined ? options.right : btnModes[mode].rightPos
        );
    ...
    function setDefaultBtnHorizontalPosition() {
      if (isTopAligned) {
        topBtnPos = setDefautlPositionProps(
          options.top !== undefined ? options.top : btnModes[mode].topPos
        );
      } else {
        bottomBtnPos = setDefautlPositionProps(
          options.bottom !== undefined
            ? options.bottom
            : btnModes[mode].bottomPos
        );
      }
    ...
    function setTextPanelPosition(el, topPos) {
      ...
      if (isLeftAligned) {
        ...
        el.style.right = "";
        el.style.left =
          (absolutePosition ? 0 : parseInt(rootEl.style.left, 10)) +
          _btnSize +
          10 +
          "px";
      } else {
        ...
        el.style.left = "";
        el.style.right =
          (absolutePosition ? 0 : parseInt(rootEl.style.right, 10)) +
          _btnSize +
          10 +
          "px";
      }
      ...
      if (!topPos) {
        if (isTopAligned) {
          el.style.bottom = "";
          el.style.top =
            (absolutePosition ? 0 : parseInt(rootEl.style.top, 10)) +
            _btnSize / 2 +
            "px";
        } else {
          el.style.top = "";
          el.style.bottom =
            (absolutePosition ? 0 : parseInt(rootEl.style.bottom, 10)) +
            _btnSize / 2 +
            "px";
        }
      }
    ...
    function applyBtnSizeOptions(size) {
      ...
      if (isMobile()) {
        recognisedTextHolder.style.maxWidth =
          "calc(100vw - " +
          (parseInt(sideBtnPos, 10) + parseInt(btnSize, 10) + 20) +
          "px)";
      }
    ...
    listenStateBtnIconImg.alt =
      alanAltText + " button icon for listening state";
    processStateBtnIconImg.alt =
      alanAltText + " button icon for processing state";
    ...
    disconnectedMicLoaderIconImg.alt =
      alanAltText + " disconnected microphone icon";
    disconnectedMicLoaderIconImg.src =
    ...
    function createAlanStyleSheet(btnOptions) {
      ...
      if (options.shadowDOM) {
        existingStyleSheet = options.shadowDOM.getElementById(
          "alan-stylesheet-" + projectId
        );
      } else {
        existingStyleSheet = document.getElementById(
          "alan-stylesheet-" + projectId
        );
      }
      ...
      keyFrames +=
        getStyleSheetMarker() +
        " .alanBtn-recognised-text-holder.with-text.left-side { text-align: left;}";
      keyFrames +=
        getStyleSheetMarker() +
        " .alanBtn-recognised-text-holder.with-text.right-side { text-align: right;}";
      ...
      keyFrames +=
        getStyleSheetMarker() + ".shadow-appear {  opacity: 1 !important;  }\n";
      keyFrames +=
        getStyleSheetMarker() +
      ...
      keyFrames +=
        ...
        roundedTriangleSecondLayerSrc +
        "); pointer-events: none;}";
      keyFrames +=
        getStyleSheetMarker() +
        ".circleMicIconBg {background-image:url(" +
        circleSecondLayerSrc +
        "); pointer-events: none;}";
      ...
      keyFrames +=
        getStyleSheetMarker() +
        ".alan-overlay-popup.default-popup {border-radius:10px; box-shadow: 0px 5px 14px rgba(3, 3, 3, 0.25);padding:6px 30px 6px 12px;text-align: left;width: 220px;background: rgb(255 255 255);}";
      keyFrames +=
        getStyleSheetMarker() +
        ".alan-overlay-popup.top.right {border-top-right-radius: 0!important;}";
      keyFrames +=
        getStyleSheetMarker() +
        ".alan-overlay-popup.top.left {border-top-left-radius: 0!important;}";
      keyFrames +=
        getStyleSheetMarker() +
        ".alan-overlay-popup.bottom.left {border-bottom-left-radius: 0!important;}";
      keyFrames +=
        getStyleSheetMarker() +
        ".alan-overlay-popup.bottom.right {border-bottom-right-radius: 0!important;}";
      keyFrames +=
        getStyleSheetMarker() +
      ...
      keyFrames +=
        getStyleSheetMarker() + ".alan-overlay-popup__ok:hover {opacity:0.9}";
      keyFrames +=
        getStyleSheetMarker() +
      ...
      keyFrames +=
        getStyleSheetMarker() +
        generateKeyFrame("alan-fade-in", "0%{opacity: 0;}100%{opacity:1;}");
      keyFrames +=
        getStyleSheetMarker() +
        generateKeyFrame("alan-fade-out", "0%{opacity: 1;}100%{opacity:0;}");
      ...
      for (i = 0; i < states.length; i++) {
        ...
        for (var j = 0; j < stateNameClasses.length; j++) {
          ...
          if (tempLayer.background) {
            keyFrames +=
              getStyleSheetMarker() + ".alanBtn-bg-" + stateNameClass + " {";
            keyFrames +=
              "background-image: linear-gradient(122deg," +
              tempLayer.background.color[0] +
              "," +
              tempLayer.background.color[1] +
              ");";
            keyFrames += "}";
            keyFrames +=
              getStyleSheetMarker() +
          ...
          if (tempLayer.hover) {
            ...
            keyFrames +=
              "background-image: linear-gradient(122deg," +
              tempLayer.hover.color[0] +
              "," +
              tempLayer.hover.color[1] +
              ");";
            keyFrames += "}";
            keyFrames +=
              getStyleSheetMarker() +
    ...
    function showPopup(popupOptions, keepPopupOverlay) {
      ...
      if (popupOptions.style) {
        ...
        popupStyle.innerHTML = popupOptions.style.replace(
      ...
      if (!absolutePosition) {
        if (!isLeftAligned) {
          ...
          popup.style.left =
            rootElClientRect.x + (-buttonMarginInPopup || 0) + "px";
        }

        if (rootElClientRect.top > 80) {
          popup.classList.add("bottom");
          popup.style.top =
            rootElClientRect.top +
            (buttonMarginInPopup
              ? _btnSize + buttonMarginInPopup
              : -popup2BtnMargin) +
            "px";
          ...
          popup.style.top =
            ...
            (buttonMarginInPopup
              ? -buttonMarginInPopup
              : _btnSize + popup2BtnMargin) +
            "px";
        }
      } else {
        popup.style.position = "absolute";
        popup.style[isLeftAligned ? "left" : "right"] =
          (-buttonMarginInPopup || 0) + "px";
        popup.style[isTopAligned ? "top" : "bottom"] =
          (buttonMarginInPopup
            ? -buttonMarginInPopup
            : _btnSize + popup2BtnMargin) + "px";
      ...
      if (!popupOptions.html) {
        if (message) {
          ...
          popup.innerHTML =
            '<div class="alan-overlay-popup__body">' + message + "</div>";
        }
      } else {
    ...
    function switchState(newState) {
      ...
      if (
        (newState === LISTENING && !listenStateBtnIconImg.src) ||
      ...
      if (
        ...
        newState === NO_VOICE_SUPPORT ||
        newState === NOT_SECURE_ORIGIN
      ) {
